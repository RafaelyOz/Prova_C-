...
builder.Services.AddScoped<ServicoService>();// Adicionando ServicoService
...
//endpoint Servico
app.MapPost("/servicos", async (Servico servico, ServicoService servicoService, HttpContext context) =>
{
...
// Atualizar dados    
app.MapPut("/servicos/{id}", async (int id, Servico servicoAtualizado, ServicoService servicoService, HttpContext context) =>
{
    if (!context.User.Identity.IsAuthenticated)
    {
        return Results.Unauthorized();
...
//endpoint contrato 
app.MapPost("/contratos", async (Contrato contrato, ContratoService contratoService, HttpContext context) =>
{
    if (!context.User.Identity.IsAuthenticated)
    {
        return Results.Unauthorized();
...
//consultar contrato
app.MapGet("/clientes/{clienteId}/servicos", async (int clienteId, ContratoService contratoService, HttpContext context) =>
{
    if (!context.User.Identity.IsAuthenticated)
    {
        return Results.Unauthorized();
